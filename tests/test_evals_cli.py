...
from tests.test_evals_runner import install_fake_claude
...
def isolated(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("QI_EVAL_CLAUDE_BIN", str(install_fake_claude(tmp_path)))
...
def test_run_single_case_with_repetitions(isolated):
    ...
    assert len((isolated / "runs.jsonl").read_text(encoding="utf-8").splitlines()) == 2
...
def test_label_walks_pending_runs_and_records_verdicts(isolated):
    ...
    assert result.exit_code == 0, result.output
    labels = [json.loads(l) for l in (isolated / "labels.jsonl").read_text(encoding="utf-8").splitlines()]
    assert len(labels) == 1
